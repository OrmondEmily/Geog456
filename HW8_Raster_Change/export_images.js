...
var datasetMore = ee.ImageCollection('USDA/NAIP/DOQQ')
                .filter(ee.Filter.bounds(geometry))
                .filter(ee.Filter.date(oneMore + '-01-01', oneMore + '-12-31'))
...
var moreImg = datasetMore.select(['R', 'G', 'B'])
moreImg = moreImg.median().clip(geometry);
...
Export.image.toDrive({
    image: result,
    description: 'Histogram Match' + yearBefore,
    scale: 0.6,
    region: geometry,
    format: 'JPEG'
})
...
Export.image.toDrive({
  image: result2,
  description: 'Histogram Match' + oneMore,
  scale: 0.6,
  region: geometry,
  format: 'JPEG'
 })